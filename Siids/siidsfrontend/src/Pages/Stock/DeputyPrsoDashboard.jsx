...
const DeputyPrsoDashboard = () => {
  ...
  const filteredGoods = useMemo(() => {
    let list = activeTab === 0 
      ? goodsList.filter(i => i.status === 'PENDING_RELEASE')
      : goodsList.filter(i => i.status === 'PENDING_PRSO_RELEASE_APPROVAL' || i.status === 'RELEASED' || i.status === 'RELEASED_FROM_MAIN');
  ...
  const handleVerifyRelease = async () => {
    try {
      await stockApi.verifyReleaseMachine(selectedItem.id);
      toast.success('Release Verified and Escalated successfully');
      handleActionSuccess();
    } catch (err) {
  ...
  return (
    <Box sx={{ p: 4, bgcolor: 'var(--surface-page)', minHeight: '100vh' }}>
      ...
      <Box sx={{ mb: 4 }}>
        <Typography variant="h4" fontWeight={800} color="var(--gray-900)">Deputy PRSO Verification Dashboard</Typography>
        <Typography variant="body1" color="var(--gray-500)">Verify release notes before escalating to PRSO for final authorization</Typography>
      </Box>

      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Tabs value={activeTab} onChange={(e, val) => { setActiveTab(val); setPage(0); }}>
          <Tab label={`Pending Verification (${goodsList.filter(i => i.status === 'PENDING_RELEASE').length})`} sx={{ textTransform: 'none', fontWeight: 600 }} />
                    ...
                    <tr key={item.id} style={{ borderTop: '1px solid rgba(0,0,0,0.05)', transition: 'background 0.2s' }}>
                      ...
                      <td style={{ padding: '16px 24px' }}>
                        {item.status === 'PENDING_RELEASE' ? (
                          <Chip label="Awaiting Verification" color="warning" size="small" variant="outlined" icon={<HourglassEmpty fontSize="small"/>} />
                        ) : (
                           <Chip label="Verified (Escalated)" color="success" size="small" icon={<CheckCircle fontSize="small"/>} />
                        )}
                      </td>
      ...
      <RightDrawer
        ...
        footerActions={
          (selectedItem?.status === 'PENDING_RELEASE') && (
        ...
        {selectedItem && (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <Alert severity="info" sx={{ borderRadius: 2 }}>
              Verification step required. Review original Seizure Note and Statement of Offence before escalating to PRSO.
            </Alert>
      ...
      <ConfirmDialog
        ...
        onConfirm={handleVerifyRelease}
        title="Verify & Escalate Release"
        body={`By approving, you verify the release details for ${selectedItem?.recipientName || 'the recipient'} and escalate it to the PRSO for final sign-off.`}
        variant="success"
        confirmLabel="Verify & Escalate"
      />
      ...
      <ConfirmDialog
        ...
        title="Deny Release Verification"
...
export default DeputyPrsoDashboard;
